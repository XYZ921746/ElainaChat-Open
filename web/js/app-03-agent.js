...
async function runAgentPhoneOperation(raw) {
    ...
    // 还没选实现方式 → 手机操作不可用（与「设置 → 高级 → 手机操作」里的状态提示一致）
    if (!currentBackendId()) {
        insertAgentResult('已跳过：还没有选择「实现方式」，AI 现在不能操作手机。'
            + '请到「设置 → 高级 → 手机操作」点「选择实现方式」选一种。');
        return;
    }
    // 总开关关掉时直接拒绝（设置 → 高级 → 手机操作）
    if (state.settings.agentPhoneEnabled === false) {
        insertAgentResult('已跳过：手机操作总开关是关闭的。需要时请在 设置 → 高级 → 手机操作 里打开「允许 AI 操作手机」。');
        return;
    }
...
// ===== 实现方式（后端选择）=====
// 四种执行方式，能力与门槛不同。入口在「设置 → 高级 → 手机操作」里（不在侧边栏：
...
function agentPhoneSkillText() {
    ...
    if (!backend || !enabled) {
        return '# Agent Skill：手机操作（当前未启用）\n'
            + '这台设备支持手机操作，但用户还没有在「设置 → 高级 → 手机操作」里选好「实现方式」'
            + (enabled ? '' : '，或「允许 AI 操作手机」总开关是关闭的') + '。\n'
            + '因此你现在**不能**操作手机，也**不要**输出 [操作:手机…] 标签（会被系统直接拒绝，等于白说）。\n'
            + '如果用户希望你操作手机，请如实说明还没有启用，并引导他去「设置 → 高级 → 手机操作」'
    ...
    if (isAccessibility) {
        ...
        lines.push('  要启动某个应用，请先用 [操作:手机查看界面] 找到它的图标，再点击图标坐标；'
            + '或请用户到「设置 → 高级 → 手机操作」换成 Shizuku / Root / 模块 方式。');
