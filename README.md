# ElainaChat Open
# 本改版项目目前属于测试状态会有未知bug请等待作者更新，感谢你的支持和理解。
...
### 鼠标跟随：关闭通话后再打开会丢监听
鼠标追踪的 `pointermove` 监听器原本只在 `buildUI()` 里挂载，而 `buildUI()` 只在**页面首次打开通话**时执行一次；`close()` 会移除监听器并置空。于是「打开通话 → 关闭 → 再打开」之后监听器再也不会重新挂载，**鼠标跟随永久失效**——这正好命中"关掉通话去设置里上传新模型、然后重新打开"这条常见路径，很容易被误判成"新模型不支持跟随"。

修复：抽出幂等的 `ensureMouseTracking()` 在每次 `open()` 时调用；监听范围扩到整个通话层（鼠标在顶栏/底栏附近也能跟随）；拖拽调整框时跳过。另加了一条提示：跟随关闭时通话界面显示「鼠标跟随已关闭 · 点此开启」，可直接点击开启。

### 安全与稳定性加固（一轮完整代码审计后）
服务端与前端都做了逐行审计，修复要点：

- **防 DNS rebinding**：新增 Host 白名单（`localhost` / `127.0.0.1` / `::1` / 本机网卡地址）。否则攻击者域名解析到 127.0.0.1 就能让浏览器请求被当成"本机管理员"，拿到任意文件读写。用主机名/域名访问时用 `ALLOWED_HOSTS=你的域名` 追加。
- **跨站 GET 拦截**：`/api/*` 不再区分方法做同源校验（`Sec-Fetch-Site`，缺失时退回 `Origin` 对比）。此前 `/api/agent/read` 是 GET，任意网页都能借本机浏览器读本地文件。
- **zip 解压重写**：改为以**中央目录**为权威元数据，不再靠扫描 `PK\x07\x08` 签名定位 data descriptor —— 旧实现遇到数据里恰好含该 4 字节的包会**静默截断并丢掉后续条目**。同时加了**解压后体积上限**（旧实现只限制压缩包大小，实测 119.8 KB 的包能落盘 120 MB）。
- **危险扩展名拦截**：zip 里的 `.html/.js/.svg/.exe` 等不再落盘；`live2d/models/` 下一律按 `application/octet-stream` 返回；全站 `nosniff`。
- **`computer` 模式写文件前确认**：此前 AI 可无声覆盖任意路径。
- **其他**：`agentWrite` 请求体上限、密码校验改异步 scrypt（避免阻塞事件循环）、会话与失败计数定期清理、静态文件流补齐错误处理、符号链接不跟随、端口占用给出可读提示。
- **前端**：标签统一"最后一个生效"（不再出现同类标签只认第一个）、表情切换/模型加载加并发序号（避免状态互相覆盖与实例泄漏）、去掉重复渲染、背景值消毒（AI 给的外链不再写进 CSS）、静音加界面提示、表情/动作名拒绝 `..` 路径、模型无 `exp` 目录时不再发注定 404 的请求。

> 全部改动都有回归验证：服务端 39 项 + 前端 25 项，均通过。实测细节见 [`docs/交付文档.md`](docs/交付文档.md) 第 8.9 节。
...
>  本仓库**不含** `android-app/` 目录，上述改动不在本仓库内。
...
> 服务只接受**本机 / 局域网地址 / localhost** 的 Host 头（防 DNS rebinding）。如果你要用主机名或域名访问，请先设置 `ALLOWED_HOSTS=你的域名` 再启动；用不到就直接用 IP 访问。

Web 版没有任何中转服务。如果服务商拒绝浏览器跨域请求，可以使用 Android 版，或在“自定义 Base URL”中填写自己部署并信任的 HTTPS 服务。

## Android 端

>  本仓库**不包含** `android-app/`（Capacitor + Gradle 打包工程，已在 `.gitignore` 中排除），需要自行补回该目录后才能构建；否则 `npm run sync:web` / `npm run build:apk` 会失败。
...
- 鼠标跟随幅度可在「设置 → Live2D」调整：滑块 0-10（×0.0~×1.0）+ 数字框解锁到 100（×10）。头部转动幅度受**模型自身参数量程**限制（多数模型约 ×1.5 到顶），更高倍率由「身体跟随」补足
- 尺寸记忆按模型名存储，手机 / 桌面共用一份，跨端切换屏幕后比例会按当前屏幕高度钳制
- 安卓（Capacitor）端 Live2D 未实测（网格手柄交互同样未实测）
- 上传大 zip（>50MB）内存占用较高（整包读入内存）
- 仓库内置默认模型 `web/live2d/models/伊蕾娜·默认/`（版权说明见下）；**其余模型不入库，用户自行上传的模型版权需自行注意**
...
**Q: 换了新模型后鼠标不动了？**
A: 先看「设置 → Live2D → 鼠标跟随」是否开着——关闭时通话界面会显示「鼠标跟随已关闭 · 点此开启」，点一下即可。如果开关是开的还是不动，用 `diag-param-bind.html`（`http://127.0.0.1:4173/diag-param-bind.html`）扫一下该模型的 `ParamAngleX` / `ParamAngleY` 是否真正绑定到绘制——有些模型作者会把标准参数名留空（参数存在但设了没反应）。
...
仓库内 `web/live2d/models/伊蕾娜·默认/` 是为了让程序 clone 下来即可运行而附带的默认模型。**该模型的著作权属于其原作者，不在本项目 MIT 许可范围内**，本项目仅作展示/个人使用之用途附带分发。若你计划商用或再分发，请自行联系模型作者取得授权；如原作者提出异议，我会立即移除。Live2D 运行库（Cubism SDK）的使用另受 Live2D Inc. 的条款约束。
