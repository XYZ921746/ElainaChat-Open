...
> ⚠️ 本分支**不包含** `android-app/`（Capacitor + Gradle 打包工程，已从仓库排除），因此下面的构建步骤需要自行补回该目录后才能使用；`npm run sync:web` / `npm run build:apk` 也会因缺少该目录而失败。
...
### 新增功能
- Live2D 视频通话增强：鼠标跟随（头部/眼睛追踪）、表情/情绪自然语言触发、跨模型表情自适应、通用水印控制、调整框
- AI 定时任务（未来消息）：AI 创建 [任务:...] 自动到点发消息
- Agent 文件操作：自然语言触发写文件、权限分级（应用内限制 / 允许操作电脑）、AI 自然解释报错、无删除权限
- 视觉模型多模态直答：图片 → 多模态模型 → 主模型角色修正
